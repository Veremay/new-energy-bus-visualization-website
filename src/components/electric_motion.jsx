...
//   const carPosition = Math.min((scrollPosition / maxScroll) * maxCarPosition, maxCarPosition);
  ...
  const carPosition = (() => {
    ...
    if (scrollPosition < (maxScroll * 3 / 8)) {
      return ((scrollPosition / (maxScroll * 3 / 8)) * maxCarPosition) * speedFactor;
    } else if (scrollPosition < (maxScroll * 4 / 8)) {
      return (((scrollPosition - (maxScroll * 3 / 8)) / (maxScroll * 1 / 8)) * maxCarPosition - 500) * speedFactor;
    } else if (scrollPosition < (maxScroll * 5 / 8)) {
      return (((scrollPosition - (maxScroll * 4 / 8)) / (maxScroll * 1 / 8)) * maxCarPosition - 500) * speedFactor;
    } else if (scrollPosition < (maxScroll * 6 / 8)) {
        return (((scrollPosition - (maxScroll * 5 / 8)) / (maxScroll * 1 / 8)) * maxCarPosition - 500) * speedFactor;
      } else if (scrollPosition < (maxScroll * 7 / 8)) {
        return (((scrollPosition - (maxScroll * 6 / 8)) / (maxScroll * 1 / 8)) * maxCarPosition - 500) * speedFactor;
      } else if (scrollPosition < maxScroll) {
        return (((scrollPosition - (maxScroll * 7 / 8)) / (maxScroll * 1 / 8)) * maxCarPosition - 500) * speedFactor;
      }
...
//   };
  ...
  return (
    ...
    >
      {
        trams.map((currentTram, tramIndex) => tramIndex === currentCarIndex ?(
          ...
          <React.Fragment key={tramIndex}>
            ...
            {currentTram?.back_wheels && (
              <div style={{ 
                ...
                marginLeft: tramIndex === 3 ? "25%" : tramIndex === 4 ? "30%" : tramIndex === 5 ? "30%" : "6%",
                zIndex: 2 }}>
                {currentTram.back_wheels}
              </div>
            )}
            {currentTram?.front_wheels && (
              <div style={{ 
                marginTop: tramIndex === 3 ? "-13%" : tramIndex === 4 ? "-13%" : tramIndex === 5 ? "-13%" : "-6%",
                marginLeft: tramIndex === 3 ? "-13%" : tramIndex === 4 ? "-13%" : tramIndex === 5 ? "-8%" : "6%",
                zIndex: 4, }}>
                {currentTram.front_wheels}
              </div>
            )}
